...
function onMapLoaded(map: mapboxgl.Map) {
  ...
  function updateArea(e) {
    ...
    map.on("click", "gl-draw-polygon-fill.cold", function (e) {
      var description =
        "<form > Enter name: <input type='text' id='name'/></br> Enter Description:<input type='description'/><button value='submit'  id='view-full'>Submit</button></form > ";
      new mapboxgl.Popup().setLngLat(e.lngLat).setHTML(description).addTo(map);
      console.log("this is come from line nu-112", description);
    });
  }
  function handlePop() {
    console.log("Helo");
